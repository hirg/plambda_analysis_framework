void fullep_flatness_test(TFile* f);
void halfep_flatness_test(TFile* f);
void resolution_test(TFile*, Double_t&);
void protonv2_test(TFile*, Double_t ep_res);
void proton_flatness_test(TFile* f);
void lambda_flatness_test(TFile* f);
void lambda_bin_flatness_test(TFile* f);
void lambdav2_test(TFile* f, Double_t ep_res);
...
void qa_plots(){
    ...
    fullep_flatness_test(f);   

    // Sub-event plane flattening test
    halfep_flatness_test(f);   

    // Event plane resolution
    resolution_test(f, ep_res);

    // Proton v2
    protonv2_test(f, ep_res);   

    // Lambda v2
    lambdav2_test(f, ep_res);   

    // Proton phi distribution
    proton_flatness_test(f);

    // Lambda phi distribution
    lambda_flatness_test(f);

    // Lambda phi distribution differential version
    lambda_bin_flatness_test(f);
}
...
void resolution_test(TFile* f, Double_t& ep_res){
    ...
    std::cout << "Full event plane resolution is: " << real_resolution << std::endl;
    ep_res = real_resolution;
}

void protonv2_test(TFile* f, Double_t ep_res){
...
void lambdav2_test(TFile* f, Double_t ep_res){
    ...
    gPad->SetTicks(1, 1);
    gPad->SaveAs("lambdav2.pdf");

    // Nasim publication
    const int size = 12;
    Double_t x[size] = {0.455, 0.559, 0.655, 0.759, 0.860, 0.955, 1.103, 1.297, 1.488, 1.680,  1.880, 2.095};
    Double_t v2[size] = {0.017, 0.026, 0.036, 0.50, 0.062, 0.078, 0.1, 0.131, 0.159, 0.185, 0.208, 0.227};
